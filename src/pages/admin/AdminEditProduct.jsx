...
import {
  ShieldCheck, LogOut, Package, Save, X, Upload, Link as LinkIcon, AlertCircle
} from 'lucide-react'
import { api } from '../../utils/api'

const CATEGORIES = ['مواد تموينية', 'أدوات منزلية', 'مستحضرات تجميل']

function AdminEditProduct({ user, onLogout }) {
  const { id } = useParams()
  ...
  const fetchProduct = async () => {
    ...
    if (result.success) {
      ...
      if (product.image) setPreview(product.image.startsWith('/') ? `https://supermarket-api-w79n.onrender.com${product.image}` : product.image)
    }
    setLoading(false)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.imageType === 'file' && selectedFile) {
        ...
        result = await api.uploadFilePut(`/admin/products/${id}`, data)
      } else {
        result = await api.put(`/admin/products/${id}`, {
          name: formData.name,
          ...
          imageUrl: formData.imageType === 'url' ? formData.imageUrl : currentImage
        })
      }
